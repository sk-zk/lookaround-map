...
def create_app():
    ...
    # /mwtoken and /ma/bootstrap(?) are passed thru the server due to CORS
    @app.route("/generate_token")
    def generate_token():
        mwtoken = requests.get("https://maps.apple.com/place/mwtoken").text
        mwtoken = f"Bearer {mwtoken}"
        resp = requests.get("https://cdn.apple-mapkit.com/ma/bootstrap", headers={"authorization": mwtoken})
        return resp.json()
